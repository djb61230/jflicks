 ...
 * @version 1.0
 */
public class HDHRScanJob extends AbstractJob {
    ...
    public void run() {
        ...
        File f = null;
        try {
            f = File.createTempFile("hdhrscan", ".log");
        } catch (Exception ex) {
        }
        ...
        if ((usertype != null) && (!usertype.equals("auto"))) {

            config.channelMap(getId(), getTuner(), usertype);
        }
        config.scan(getId(), getTuner(), f.getPath());
        ...
        if ((array != null) && (array.length > 0)) {
            ...
            for (int i = 0; i < array.length; i++) {
                ...
                if (freq != -1) {
                    ...
                    foundlist.add(ref);
                    String line = array[i].getNumber() + "=" + ref + ":" + freq;
                    sb.append(line);
                    sb.append("\n");
                ...
                for (int i = 0; i < keys.length; i++) {
                    ...
                    if (!foundlist.contains(keys[i])) {
                        ...
                        if (freq != -1) {
                            ...
                            String line = "UNKNOWN_VIRTUAL_" + index + "=" + keys[i] + ":" + freq;
                            sb.append(line);
                            sb.append("\n");
                            index++;
                        }
            ...
            if (sb.length() > 0) {
                ...
                if ((conf.exists()) && (conf.isDirectory())) {
                    ...
                    try {
                        ...
                        log(LogUtil.DEBUG, "Writing " + scan.getPath());
