...
public class VideoInfoWindow extends JWindow implements ActionListener {
    ...
    public VideoInfoWindow(Rectangle r, int seconds, Color normal,
        ...
        if (Util.isTranslucencySupported()) {
            ...
            Util.setWindowOpacity(this, alpha);
        }

        // See if we can shape our window...
        if (Util.isShapedSupported()) {

            double dwidth = (double) width;
            double dheight = (double) height;
            double arc = dheight / 10.0;
            RoundRectangle2D.Double rrect = new RoundRectangle2D.Double(
                0, 0, (double) width, (double) height, arc, arc);
            Util.setWindowShape(this, rrect);
        }
    }
