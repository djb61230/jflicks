...
#
# Modified by Jeff Simpson November 2005 to support D11 USB serial connection
#
#
...
# Home site for this version http://www.pdp8.net/directv/directv.shtml
...
#
# Modified by David Gesswein < djg@pdp8.net > June 22, 2005
...
@hdChannels = ("702", "703", "704", "705", "706", "707", "708", "709", "710",
...
sub sendbytes {
  ...
  #if ( !$port_opened )
  #{
  #  $port = init_serial($serport, $baudrate);
  #  $port_opened = 1;
  #}
...
sub get_reply() {
    ...
    while (1) {       
       ($rc,$buf) = $serial_port->read(1);
       if ($rc < 0) {
          print STDERR "Read Error ($rc)\n" if ($verbose);
          last;
       }
       if ($rc == 0) {
          if (tv_interval($starttime) > 2) {
          #if (time() - ($starttime) > 2) {
...
sub init_serial
{
  ...
  $serial_port = Device::SerialPort->new( $serport );
  if (!$serial_port)
  {
  ...
  $serial_port->handshake("xoff");

  $serial_port->read_char_time(0);     # don't wait for each character
  $serial_port->read_const_time(1); # 1 second per unfulfilled "read" call
