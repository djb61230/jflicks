 ...
 */
void throwIOExc(JNIEnv *env, char *msg){
	if (io_exc_cls == NULL) {
 ...
 */
int unlink_if_socket(JNIEnv *env, char *path) {
	struct stat stbuf;
...
JNIEXPORT void JNICALL Java_org_lirc_socket_UnixSocketImpl_bind
	...
	socket_name = (*env)->GetStringUTFChars(env, jmyPath, 0);
...
#ifdef DEBUG
	printf("junixsocket: accept(%i, 0x%x, 0x%x)\n", fd, &client_addr, &client_addr_len);
#endif
...
JNIEXPORT void JNICALL Java_org_lirc_socket_UnixSocketImpl_connect
	...
	const char *socket_name;
	addr.sun_family = AF_UNIX;
	socket_name = (*env)->GetStringUTFChars(env, jpath, 0);
	strcpy(addr.sun_path, socket_name);
